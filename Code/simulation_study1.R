...
#set the following job information 
job_name=args[1] 
...
#sample data from differet family
set.seed(123)  # for reproducibility
if (stype=="A") {
  amh_copula1 <- amhCopula(dim = 2, param = 0.8)
  amh_copula2 <- amhCopula(dim = 2, param = -0.8)
  sample_data1 <- rCopula(sample_size, amh_copula1)
  sample_data2 <- rCopula(sample_size, amh_copula2)
} else if(stype == "C") {
  clayton_copula1 <- claytonCopula(dim = 2, param = 10)
  clayton_copula2 <- claytonCopula(dim = 2, param = -0.5)
  sample_data1 <- rCopula(sample_size, clayton_copula1)
  sample_data2 <- rCopula(sample_size, clayton_copula2)
} else if(stype == "F") {
  frank_copula1 <- frankCopula(dim = 2, param = 5)
  frank_copula2 <- frankCopula(dim = 2, param = -5)
  sample_data1 <- rCopula(sample_size, frank_copula1)
  sample_data2 <- rCopula(sample_size, frank_copula2)
} else if (stype == "J") {
  joe_copula1 <- joeCopula(dim = 2, param = 10)
  joe_copula2 <- joeCopula(dim = 2, param = 2)
  sample_data1 <- rCopula(sample_size, joe_copula1)
  sample_data2 <- rCopula(sample_size, joe_copula2)
} else if (stype == "G") {
  gumbel_copula1 <- gumbelCopula(dim = 2, param = 10)
  gumbel_copula2 <- gumbelCopula(dim = 2, param = 5)
  sample_data1 <- rCopula(sample_size, gumbel_copula1)
  sample_data2 <- rCopula(sample_size, gumbel_copula2)
}

data <- rbind(sample_data1, sample_data2)
...
#initialize the value for each parameters, theta, a, b
...
print("Finish: get estimated joint density")
